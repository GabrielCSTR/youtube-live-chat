import { YoutubeLiveChat } from "../src"
import { ChatMessage } from "../src/types/data"

jest.mock("../src/utils/requests")
import { fetchChat, fetchLivePage } from "../src/utils/request"
...
describe("YoutubeLiveChat", () => {
  ...
  test("Constructor: LiveID", () => {
    const liveId = "liveId"
    const liveChat = new YoutubeLiveChat({ liveId: liveId })
    expect(liveChat).toBeInstanceOf(YoutubeLiveChat)
    expect(liveChat.liveId).toBe(liveId)
  })

  test("Constructor: ChannelID", () => {
    const liveChat = new YoutubeLiveChat({ channelId: "channelId" })
    expect(liveChat).toBeInstanceOf(YoutubeLiveChat)
  })
  ...
  test("Constructor: No IDs Error", () => {
    ...
    // @ts-ignore
    expect(() => new YoutubeLiveChat()).toThrow("Required channelId or liveId or handle.")
  })
  ...
  test("On chat", async () => {
    ...
    const chatItem = await new Promise((resolve) => {
      onChat.mockImplementation((chatItem: ChatMessage) => {
        resolve(chatItem)
      })
    })
